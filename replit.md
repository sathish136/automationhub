...
- **Site Running Data Reports Added (August 2025)**: Implemented comprehensive site running data report functionality that connects directly to SQL databases. Added new report type that allows users to select any available database and table from the SQL viewer, with flexible time ranges (today, yesterday, week, month, custom). Reports generate formatted HTML previews with real data from production databases, showing actual operational metrics and site running data. Includes proper error handling and database/table validation.
- **Reports PDF Preview Enhanced (August 2025)**: Converted report generation from direct CSV downloads to formatted PDF preview system. Reports now show in dialog with professional HTML table formatting, color-coded status indicators, and print functionality. All 7 report types now generate print-ready previews with proper headers, timestamps, and site filtering information.
